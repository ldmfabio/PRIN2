...
> Vale também mencionar que, para que a branch `development` não seja apagada **nunca**, é preciso criar uma regra para que isso não aconteça. Para isso, basta ir até as configurações do repositório, clicar em `Branches` e, em seguida, em `Branch protection rules`. Após isso, basta criar uma regra para que a branch `development` não seja apagada. O processo é semelhante para a branch `main` ou `master`. A Figura abaixo mostra como criar uma regra para a branch `development`. No passo 1 é onde se encontra o Settings do repositório. No passo 2 é onde se encontra a opção de Branches. No passo 3 é onde se encontra a opção para adicionar regra para as `branches`. E, após adicionar a regra, ela será listada na tela, como a regra que foi criada para a branch `development`, disposta ao centro da Figura.

> ![Regra para a branch development](img/branch-rule.png)
